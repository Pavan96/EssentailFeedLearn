...
class RemoteFeedLoaderTest: XCTestCase {
    ...
    func test_load_deliversErrorOnClientError() {
        ...
        expect(sut, toCompleteWith: .failure(RemoetFeedLoader.Error.connectivity)) {
    ...
    func test_load_deliversErrorOnNon200HTTPResponse() {
        ...
        samples.enumerated().forEach { index, code in
            expect(sut, toCompleteWith: .failure(RemoetFeedLoader.Error.invalidData)) {
    ...
    func test_load_doesNotDeliverResultAfterSUTInstacneHasBeenDeallocated() {
        let url = URL(string: "http://any-url.com")!
        let client = HTTPClientSpy()
        var sut: RemoetFeedLoader? = RemoetFeedLoader(client: client, url: url)
    ...
    private func expect(_ sut: RemoetFeedLoader, toCompleteWith expectedResult: RemoetFeedLoader.Result, when action: () -> Void, file: StaticString = #file, line: UInt = #line) {
        ...
        sut.load { receivedResult in
            ...
            case let (.failure(receivedError), .failure(expectedError)):
                XCTAssertEqual(receivedError as! RemoetFeedLoader.Error, expectedError as! RemoetFeedLoader.Error, file: file, line: line)
